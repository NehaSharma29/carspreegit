...
class BrandsController extends AppController {
    ...
    public function index() {
        ...
        $data = $this->Brands->find()->order(['id'=>'DESC'])->hydrate(false)->toArray();
        $resultData = array();
        if(!empty($data)){
			foreach($data as $key => $record){
				$resultData['id'] = 60;
				$resultData['firstName'] = 'Lou';
				$resultData['lastName'] = 'Conner';
				$resultData['username'] = '@Sanchez';
				$resultData['email'] = 'lousanchez@comtours.com';
				$resultData['age'] = 16;
			}
		}
        //~ pr($resultData);
        //~ die;
        $string = '[{"id": 1,"firstName": "Mark","lastName": "Otto","username": "@mdo","email": "mdo@gmail.com","age": "28"}]';
        echo $string;die;
        //~ $this->data = $data;
        //~ return $this->data;
        //~ $this->header=200;
